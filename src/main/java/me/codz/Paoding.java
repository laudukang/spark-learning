...
public class Paoding {
	...
	public static void main(String[] args) throws IOException {
		//justSplit("你吃饭了吗");
		//inMemoryShortAnalyzer();
		//analysisCompare("你吃饭了吗");
		//anaylyzerWords("你好，你吃饭了吗，我你他，我们要好好的");
		runWithSpark();
	}

	private static void runWithSpark() {
		...
		}).flatMap(v -> Arrays.asList(v).iterator()).mapToPair((PairFunction<String, String, Integer>) s -> new Tuple2(StringUtils.trim(s), 1)).reduceByKey((Function2<Integer, Integer, Integer>) (x, y) -> x + y);

		URL url = WordCount.class.getClassLoader().getResource("");
		String savePath = new File(url.getFile()).getAbsolutePath() + File.separator + "PaodingTest";
	...
	public static List<String> anaylyzerWords(String str) throws IOException {
	...
	private static void analysisCompare(String text) throws IOException {
	...
	private static void inMemoryShortAnalyzer() {
		try {
//            Directory idx = FSDirectory.open(new File("F:/data/lucene/fix"));
	...
	public static void justSplit(String text) throws IOException {
		TokenStream ts = ANALYZER.tokenStream("text", new StringReader(text));
		ts.reset();
		// 添加工具类 注意：以下这些与之前lucene2.x版本不同的地方
		CharTermAttribute offAtt = ts.addAttribute(CharTermAttribute.class);
