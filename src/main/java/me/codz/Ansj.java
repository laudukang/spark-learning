...
public class Ansj {
	...
	private static StopRecognition fitler;

	static {
		fitler = new StopRecognition();
		//fitler.insertStopNatures("我"); //过滤单词
		//fitler.insertStopRegexes("小.*?"); //支持正则表达式
	}
	...
	private static void runWithSpark() {
		...
		String savePath = new File(url.getFile()).getAbsolutePath() + File.separator + "AnsjTest";
	...
	private static List<String> anaylyzerWords(String str) {
		List<String> list = Lists.newArrayList();

		Result result = ToAnalysis.parse(str).recognition(fitler);
		result.getTerms().forEach(t -> list.add(t.getName()));

		return list;
	}

	private static void justTestAnsj() {
